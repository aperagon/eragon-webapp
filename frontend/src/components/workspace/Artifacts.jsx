...
function ArtifactCard({ artifact, index, session, setEditedArtifacts, editingIndex, setEditingIndex, editContent, setEditContent, chartRefs }) {
  ...
  const getPlotlyChart = () => {
    const chartContainer = document.querySelector(`[data-artifact-index="${index}"] .plotly-graph-div`);
  ...
  return (
    <div className="bg-black/60 rounded-xl p-6 border border-[#333333] hover:border-[#404040] transition-all duration-300" data-artifact-index={index}>
      <div className="flex items-center justify-between mb-6">
        ...
        <div className="flex items-center space-x-2">
          ...
          <Button variant="ghost" size="sm" className="text-[#cccccc] hover:text-white hover:bg-[#333333] p-2" onClick={handleCopy}>
            <Copy className="w-4 h-4" />
          </Button>
          <Button variant="ghost" size="sm" className="text-[#cccccc] hover:text-white hover:bg-[#333333] p-2" onClick={handleDownload}>
